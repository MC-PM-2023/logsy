...
# 🆕 Added 'target_hours' to arguments
def generate_weekly_email_html(user_name, team_name, week_data, total_hours, leaves, start_date, end_date, target_hours):
    
    # 🔴 Old hardcoded value removed
    # target_hours = 47.5 
    
    pct = min(100, int((total_hours / target_hours) * 100)) if target_hours > 0 else 0
    bar_color = "#10b981" if total_hours >= (target_hours * 0.85) else "#f59e0b" #
    html = f"""
    <!DOCTYPE html>
    <html>
    <body style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #0f172a; color: #e2e8f0; margin: 0; padding: 20px;">
        <div style="max-width: 600px; margin: 0 auto; background-color: #1e293b; border-radius: 12px; overflow: hidden; border: 1px solid #334155; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.5);">
            <div style="background: linear-gradient(135deg, #1e293b 0%, #0f172a 100%); padding: 25px; text-align: center; border-bottom: 2px solid #ffd24d;">
                <h2 style="color: #ffd24d; margin: 0; font-size: 24px; letter-spacing: 1px;">🔥 Weekly Progress Report</h2>
                <p style="color: #94a3b8; font-size: 13px; margin: 5px 0 0 0;">{start_date} to {end_date}</p>
            </div>

            <div style="padding: 25px;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                    <div>
                        <h3 style="margin: 0; color: #f8fafc; font-size: 20px;">{user_name}</h3>
                        <span style="font-size: 12px; color: #94a3b8; background: #0f172a; padding: 4px 8px; border-radius: 4px; border: 1px solid #334155;">{team_name}</span>
                    </div>
                    <div style="text-align: right;">
                        <span style="font-size: 28px; font-weight: 800; color: {bar_color}; display: block; line-height: 1;">{total_hours}h</span>
                        <span style="font-size: 11px; color: #64748b; text-transform: uppercase; font-weight: bold;">Total Productive</span>
                    </div>
                </div>

                <div style="background-color: #334155; height: 8px; border-radius: 4px; margin-bottom: 8px; overflow: hidden;">
                    <div style="width: {pct}%; background-color: {bar_color}; height: 100%; border-radius: 4px;"></div>
                </div>
                <div style="text-align: right; font-size: 11px; color: #94a3b8; margin-bottom: 25px;">
                    {pct}% of Weekly Target (47.5h)
                </div>

                <table style="width: 100%; border-collapse: collapse; font-size: 13px; margin-bottom: 20px;">
                    <thead>
                        <tr style="color: #64748b; font-size: 11px; text-transform: uppercase; letter-spacing: 0.5px;">
                            <th style="text-align: left; padding-bottom: 10px; border-bottom: 1px solid #334155;">Day</th>
                            <th style="text-align: right; padding-bottom: 10px; border-bottom: 1px solid #334155;">Status</th>
                            <th style="text-align: right; padding-bottom: 10px; border-bottom: 1px solid #334155;">Hours</th>
                        </tr>
                    </thead>
                    <tbody>
    """
    ...
    for day_info in week_data:
        status_color = "#ef4444" if day_info['status'] == 'Leave' else "#cbd5e1"
        row_bg = "rgba(255,255,255,0.02)" if day_info['day'] in ['Saturday'] else "transparent"
        
        html += f"""
                        <tr style="background-color: {row_bg};">
                            <td style="padding: 12px 8px; color: #cbd5e1; border-bottom: 1px dashed #334155;">{day_info['day']}</td>
                            <td style="padding: 12px 8px; text-align: right; color: {status_color}; border-bottom: 1px dashed #334155;">{day_info['status']}</td>
                            <td style="padding: 12px 8px; text-align: right; font-weight: bold; color: #fff; border-bottom: 1px dashed #334155;">{day_info['hours']}</td>
                        </tr>
        """
    ...
    html += f"""
                ...
                <div style="background: rgba(15, 23, 42, 0.5); padding: 15px; border-radius: 8px; display: flex; justify-content: space-between; align-items: center; border: 1px solid #334155;">
                    <span style="color: #94a3b8; font-size: 13px;">Productive Days: <b style="color: #fff;">{6 - leaves}</b></span>
                    <span style="background: rgba(239, 68, 68, 0.15); color: #f87171; padding: 4px 10px; border-radius: 6px; font-size: 12px; font-weight: bold; border: 1px solid rgba(239, 68, 68, 0.2);">
                        Leaves: {leaves}
                    </span>
                </div>
            </div>

            <div style="background-color: #0f172a; padding: 15px; text-align: center; color: #475569; font-size: 11px; border-top: 1px solid #334155;">
                Generated by Logsy Automation • {datetime.now().year}
            </div>
        </div>
    </body>
    </html>
...
def weekly_progress_job():
    """
    Executes Logic: Mon-Sat calc, Exclude Breaks/Permission, Leave Count.
    Sends Emails.
    """
    with app.app_context():
        ...
        today = date.today() # Assuming this runs on Saturday
        # If running on Saturday, today is end date.
        # If testing on another day, adjust accordingly.
        ...
        # Data storage for Admin summaries
        # { 'AdminEmail': [ 'html_report_user1', 'html_report_user2' ] }
        team_reports = {} 
        super_admin_reports = []
        ...
        for user in users:
            ...
            # 🛑 EXCLUDING 'Breaks' and 'Permission'
            cur.execute("""
            ...
            """, (user.username, start_str, end_str))
            
            logs = cur.fetchall()
            
            # 4. Process Data
            daily_map = {} # { '2023-10-23': 28800 seconds }
            ...
            tm = divmod(tm, 60)[0]
            total_hours_str = f"{int(th)}.{int((tm/60)*10)}" # Decimal format e.g. 40.5
            ...
            if user.team:
                ...
                for admin in admins:
                    if admin.email:
                        if admin.email not in team_reports: team_reports[admin.email] = []
                        team_reports[admin.email].append(user_html_card)

        cur.close()

        # 7. SEND EMAILS 
        
        # A. Send to Team Admins
        for admin_email, reports in team_reports.items():
            full_body = "<br><hr><br>".join(reports)
            send_email_report(admin_email, f"Weekly Team Report ({start_str})", full_body)
            
        # B. Send to Super Admins
        ...
        if super_admins and super_admin_reports:
            ...
            for sa in super_admins:
                if sa.email:
                    send_email_report(sa.email, f"Weekly Organization Report ({start_str})", full_body_super)
...
def create_scheduler():
    ...
    # sched.add_job(check_missing_entries, CronTrigger(day_of_week='mon-sat', hour=13, minute=9, timezone=ist))
    # sched.add_job(send_weekly_summary, CronTrigger(day_of_week='sat', hour=13, minute=6, timezone=ist))
    # sched.add_job(weekly_progress_job, CronTrigger(day_of_week='sat', hour=13, minute=2, timezone=ist))
    ...
    sched.add_job(weekly_progress_job, CronTrigger(day_of_week='sat', hour=18, minute=0, timezone=ist))

    # 🚀 4. NEW: Monthly Compensation Report (Last Day of Month @ 6 PM)
    def trigger_monthly_report_if_last_day():
        # Check if tomorrow is the 1st of the next month (implies today is the last day)
    ...
    # Runs every day at 6 PM, but executes logic only on the last day
    sched.add_job(trigger_monthly_report_if_last_day, CronTrigger(hour=18, minute=0, timezone=ist))
...
def calculate_dynamic_target(entries):
    """
    entries: List of dictionaries, e.g., [{'date': '2025-10-23', ...}, ...]
    """
    worked_days = set() # Set use panna duplicates irukathu
    
    # 1. First, entha days la entry vizhunthuruku nu edukrom
    for entry in entries:
        # Date string ah date object ah maathurou
        # Unga DB la already date object ah iruntha 'strptime' theva illa
        d_obj = datetime.strptime(entry['date'], "%Y-%m-%d").date()
        worked_days.add(d_obj)
    
    total_target = 0
    
    # 2. Worked days ku mattum Target add pandrom
    for day in worked_days:
        week_day = day.weekday() # 0=Mon, 1=Tue, ... 5=Sat, 6=Sun
        
        if 0 <= week_day <= 4: # Monday (0) to Friday (4)
            total_target += 8.0
        elif week_day == 5:    # Saturday (5)
            total_target += 7.5
        # Sunday or Leave days (entry illatha days) count aagaathu
...
# # #__________________________App_dep_DB_rough_______________________________
...
# ── NEW MODEL: AUDIT LOG ──────────────────────────────────────────────────
# ── NEW MODEL: AUDIT LOG ──────────────────────────────────────────────────
class ProjectAuditLog(db.Model):
    ...
    team = db.Column(db.String(100)) 
    action_type = db.Column(db.String(50)) # 'STATUS_CHANGE', 'ASSIGNED', 'REMOVED'
    previous_status = db.Column(db.String(50)) 
    new_status = db.Column(db.String(50))
    affected_user_name = db.Column(db.String(150))
    affected_user_email = db.Column(db.String(255))
...
# ── HELPER FUNCTION (MUST BE DEFINED BEFORE ROUTES) ───────────────────────
def log_audit_event(action, project_obj, admin_user, target_user=None, prev_status=None):
    try:
        log = ProjectAuditLog(
            project_code=project_obj.code,
            team=project_obj.team,
            action_type=action,
            previous_status=prev_status,
            new_status=project_obj.status, # The status AFTER the change
            assigned_by=admin_user.username,
            changed_date=datetime.now()
        )
        # If this involves a specific user (Allocation)
        if target_user:
            log.affected_user_name = target_user.username
            log.affected_user_email = target_user.email
            if action == 'ASSIGNED':
                log.assignment_start_date = datetime.now().date()
            elif action == 'REMOVED':
                log.assignment_end_date = datetime.now().date()
        
        db.session.add(log)
        # Note: We rely on the caller to perform db.session.commit()
    except Exception as e:
        print(f"Audit Log Error: {e}")
#__________________________end________________________________________________
...
def dashboard():
    ...
    cur.execute("""
        SELECT name, date, day, project, project_type, team, process, sub_process,
               start_time, end_time, duration, total_hours, project_code, project_type_mc, disease, country, Work_Type, id
        FROM   timesheetlogs
        WHERE  name = %s
        ORDER  BY id DESC
        LIMIT  30
    ...
    try:
        ...
        for r in comp_rows:
            ...
            if w_type == 'Compensation':
                ...
                if (proc == 'Breaks' and sub == 'Personal') or \
    ...
    try:
        ...
        cur.execute("""
            ...
            AND date BETWEEN %s AND %s
            AND duration IS NOT NULL
            AND process NOT IN ('Breaks', 'Permission') -- 👈 Itha kandipa add pannunga
        ...
        for r in rows:
            # Handle date type (string vs date obj)
            d_obj = r[0]
        ...
        # Default to 0 or keep calculated value
...
def admin_project_codes():
    ...
    if request.method == "POST":
        ...
        if existing:
            ...
            # 👇 2. Log Audit Event (Update)
            if prev_status != status:
                log_audit_event("STATUS_CHANGE", existing, me, prev_status=prev_status)
                
                # 👇 3. Send Slack Notification (Optional - remove if too noisy)
                assignments = UserProjectAssignment.query.filter_by(project_id=existing.id, is_active=True).all()
                for assign in assignments:
                    u_notify = User.query.get(assign.user_id)
                    if u_notify and u_notify.email:
                        msg = f"Project Status Update\nProject: {existing.code}\nNew Status: {status}\nUpdated by: {me.username}"
                        send_slack_alert(u_notify.email, msg)
            ...
            db.session.add(pc)
            # Commit first to get the ID (needed for logging sometimes, though object ref is enough usually)
            db.session.commit() 
            
            # 👇 4. Log Audit Event (Creation)
            log_audit_event("CREATED", pc, me, prev_status="NEW")
        ...
        if action in ("bulk_assign", "bulk_end"):
            ...
            for uid in user_ids:
                ...
                if action == "bulk_assign":
                    ...
                    # 👇👇 🆕 AUDIT LOG: ASSIGNED 👇👇
                    log_audit_event("ASSIGNED", project_obj, me, target_user=target_user)
                    ...
                    if exists:
                        ...
                        # 👇👇 🆕 AUDIT LOG: REMOVED 👇👇
                        log_audit_event("REMOVED", project_obj, me, target_user=target_user)
        ...
        if user_id and code_id:
            ...
            if action == "assign":
                ...
                if existing: flash("Already assigned", "info")
                    ...
                    # 👇👇 🆕 AUDIT LOG: ASSIGNED 👇👇
                    log_audit_event("ASSIGNED", code, me, target_user=target_user)
                ...
                if not existing: flash("No active assignment found", "info")
                    ...
                    # 👇👇 🆕 AUDIT LOG: REMOVED 👇👇
                    log_audit_event("REMOVED", code, me, target_user=target_user)
...
def start():
    ...
    work_type = request.form.get("work_type", "Regular") # 🆕 Added this line
...
def update_entry():
    ...
    work_type = request.form.get("work_type", "Regular") # 🆕 Added this
    ...
    try:
        ...
        # 🆕 Added work_type to params and SQL
...
def view_team_logs():
    ...
    # default team logic
    default_team = current_user.team if current_user.role != 'superadmin' else None
    
    is_post = (request.method == "POST")
    
    # 1. Capture Filters
    filter_user = (request.form.get("username") if is_post else request.args.get("username")) or None
    filter_team = (request.form.get("team") if is_post else request.args.get("team")) or default_team
    ...
    if filter_team:
        ...
        values.append(filter_team)
    
    if filter_user: 
        where_sql.append("name = %s")
    ...
    # Fetch Logs
    select_q = f"""SELECT name, date, day, project, team, process, sub_process, start_time, end_time, duration, total_hours, id 
                   FROM timesheetlogs {where_clause} 
    ...
    # Fetch Teams (Only for Superadmin)
    teams = []
    if current_user.role == 'superadmin':
        cur.execute("SELECT DISTINCT team FROM desktop_userstable WHERE team IS NOT NULL AND team != ''")
        teams = sorted([r[0] for r in cur.fetchall() if r[0]])

    # Fetch Users (CRITICAL FIX: Filter users based on selected team)
    users_query = User.query
    if filter_team:
        users_query = users_query.filter_by(team=filter_team)
    elif current_user.role != 'superadmin':
        users_query = users_query.filter_by(team=current_user.team)
        
    users = users_query.order_by(User.username).all()

    # Fetch Projects/Processes based on scope
    # (Simple approach: Fetch all distinct from logs to ensure available options are valid)
    if filter_team:
        cur.execute("SELECT DISTINCT project FROM timesheetlogs WHERE team = %s", (filter_team,))
    else:
        cur.execute("SELECT DISTINCT project FROM timesheetlogs")
    projects = sorted([r[0] for r in cur.fetchall() if r[0]])

    cur.execute("SELECT DISTINCT process FROM timesheetlogs")
    processes = sorted([r[0] for r in cur.fetchall() if r[0]])

    cur.execute("SELECT DISTINCT sub_process FROM timesheetlogs")
    sub_processes = sorted([r[0] for r in cur.fetchall() if r[0]])
    ...
    return render_template("team_logs.html", 
                           logs=logs, 
                           users=users, 
                           projects=projects, 
                           processes=processes, 
                           ...
                           selected_team=filter_team) # Pass selected team back to UI
...
def update_project_status():
    ...
    if prev_status != new_status:
        # This writes to project_audit_logs table
    ...
    return jsonify(ok=True, id=rec.id, status=rec.status, 
                   ...
                   yti_end_date=rec.yti_end_date.isoformat() if rec.yti_end_date else ""), 200
        ...
        if action == "add":
            for cid in code_ids:
                ...
                if not already:
                    ...
                    log_audit_event("ASSIGNED", pc, me, target_user=selected_user)
            ...
            for r in rows:
                ...
                # 👇👇 🆕 LOGGING ADDED HERE 👇👇
                log_audit_event("REMOVED", r.project, me, target_user=selected_user)
...
def admin_dashboard():
    ...
    val_work = cur.fetchone()[0]
    total_work_seconds = float(val_work) if val_work is not None else 0

    cur.execute(f"SELECT SUM(TIME_TO_SEC(duration)) FROM timesheetlogs {break_condition}")
    val_break = cur.fetchone()[0]
    total_break_seconds = float(val_break) if val_break is not None else 0

    # EXTRA FEATURE: Average Hours Per User
    avg_hours_per_user = 0
    if total_users > 0:
        avg_hours_per_user = round((total_work_seconds / 3600) / total_users, 2)
    ...
    # --- 4. Chart Data (Reverted: Shows ALL data now) ---
    
    # Process Distribution (All processes)
    cur.execute(f"SELECT process, SUM(TIME_TO_SEC(duration))/3600 FROM timesheetlogs {where_clause} GROUP BY process ORDER BY 2 DESC")
    process_data = cur.fetchall()

    # Projects (All projects)
    cur.execute(f"SELECT project, SUM(TIME_TO_SEC(duration))/3600 FROM timesheetlogs {where_clause} GROUP BY project ORDER BY 2 DESC")
    project_hours_data = cur.fetchall()

    # Daily Trend (Last 7 Days)
    cur.execute(f"SELECT date, SUM(TIME_TO_SEC(duration))/3600 FROM timesheetlogs {where_clause} GROUP BY date ORDER BY date DESC LIMIT 7")
    daily_data = cur.fetchall()

    # Users (All users)
    cur.execute(f"SELECT name, SUM(TIME_TO_SEC(duration))/3600 FROM timesheetlogs {where_clause} GROUP BY name ORDER BY 2 DESC")
    ...
    """)
    daily_work_break_data = [(r[0], float(r[1] or 0)/3600.0, float(r[2] or 0)/3600.0) for r in cur.fetchall()]

    cur.execute(f"SELECT sub_process, SUM(TIME_TO_SEC(duration))/3600 FROM timesheetlogs {where_clause} GROUP BY sub_process ORDER BY 2 DESC")
    ...
    if current_user.role != "superadmin": 
        user_list_query += f" WHERE team = '{current_user.team}'"
    ...
    return render_template("admin_dashboard.html", 
                           ...
                           total_break_hours=round(total_break_seconds/3600.0, 2), 
                           avg_hours_per_user=avg_hours_per_user, # Extra Metric Passed
...
def start_timer():
    ...
    # 🆕 Get Work Type from Request (Default to Regular)
    ...
    try:
        ...
        # 🆕 UPDATED SQL QUERY (Added Work_Type)
        cur.execute("""
        ...
        """, (name, start_dt.strftime("%Y-%m-%d"), start_dt.strftime("%A"), team, preset.project, proj_type_db, preset.process, preset.sub_process, start_dt.strftime("%H:%M"), proj_code, proj_type_mc, disease, country, work_type))
    ...
    # ... (Update session data if needed, usually just success response is enough)
    ...
    session["active_timer"] = {
        ...
        "is_manual": False,
        "work_type": work_type # Optional: Store in session if you want to show it in UI
    }
...
def send_monthly_compensation_report():
    ...
    Calculates pending compensation for all users.
    Sends Team-wise summary to Admins.
    Sends Full summary to Superadmins.
    """
    with app.app_context():
        print("Generaring Monthly Compensation Report...")
        today = date.today()
        # Calculate start and end of current month
        start_date = today.replace(day=1)
        # End date logic handled by getting all logs for this month/year combo
        
        # 1. Fetch All Logs for this Month
        cur = mysql.connection.cursor()
        query = """
            SELECT name, team, process, sub_process, duration, Work_Type
            FROM timesheetlogs
            WHERE MONTH(date) = %s AND YEAR(date) = %s
        """
        cur.execute(query, (today.month, today.year))
        logs = cur.fetchall()
        cur.close()

        # 2. Process Data Per User
        user_balances = {} # {username: {'team': 'X', 'balance_secs': 0}}

        for row in logs:
            name, team, process, sub, duration_str, work_type = row
            ...
            if name not in user_balances:
                user_balances[name] = {'team': team, 'balance': 0}

            # --- LOGIC SAME AS FRONTEND ---
            # If Work Type is Compensation -> Minus (Payback)
            if work_type == 'Compensation':
                user_balances[name]['balance'] -= secs
            else:
                # If Liability Conditions -> Plus (Debt)
                if (process == 'Breaks' and sub == 'Personal') or \
                   (process == 'Permission' and sub in ['Personal', 'Sick', 'Early Close']):
                    user_balances[name]['balance'] += secs

        # 3. Filter only users with Positive Balance (Pending Debt)
        pending_users = {u: data for u, data in user_balances.items() if data['balance'] > 0}

        if not pending_users:
            print("No pending compensation this month.")
            return

        # 4. Prepare Data for Emails
        # Group by Team
        team_data = {} # { 'DataTeam': [user1, user2], ... }
        full_list_html = "<h3>Full Pending Compensation List</h3><table border='1' cellpadding='5'><tr><th>Name</th><th>Team</th><th>Pending Time</th></tr>"

        for u, data in pending_users.items():
            # Format seconds to HH:MM
            b_secs = data['balance']
            h, m = divmod(b_secs, 3600)
            m = divmod(m, 60)[0]
            time_str = f"{int(h)}h {int(m)}m"
            
            # Add to full list
            full_list_html += f"<tr><td>{u}</td><td>{data['team']}</td><td style='color:red;'>{time_str}</td></tr>"
            
            # Add to team list
            tm = data['team']
            if tm:
                if tm not in team_data: team_data[tm] = []
                team_data[tm].append(f"<li><b>{u}</b>: {time_str}</li>")

        full_list_html += "</table>"

        # 5. Send Emails
        
        # A. Send to TEAM ADMINS
        for team_name, defaulters in team_data.items():
            admins = User.query.filter_by(role='admin', team=team_name).all()
            if not admins: continue
            
            body = f"<h3>{team_name} - Compensation Pending Report</h3><p>The following users have pending compensation time for {today.strftime('%B %Y')}:</p><ul>"
            body += "".join(defaulters)
            body += "</ul><p>Please ensure they compensate this time.</p>"
            
            for admin in admins:
                if admin.email:
                    send_email_report(admin.email, f"Monthly Compensation Report - {team_name}", body)

        # B. Send to SUPER ADMINS (Full List)
        super_admins = User.query.filter_by(role='superadmin').all()
        for sa in super_admins:
            if sa.email:
                send_email_report(sa.email, f"Monthly Compensation Report - All Teams", full_list_html)

# Helper for sending email (Reuse existing SMTP logic or create this)
def send_email_report(to_email, subject, html_body):
    msg = MIMEMultipart("alternative")
    msg["From"] = f"Logsy Admin <{WEBMAIL_USER}>"
    msg["To"] = to_email
    msg["Subject"] = subject
    ...
    try:
        with smtplib.SMTP(SMTP_SERVER, SMTP_PORT) as s:
            s.starttls()
            s.login(WEBMAIL_USER, WEBMAIL_PASS)
            s.sendmail(WEBMAIL_USER, to_email, msg.as_string())
